...
from pipeline.storage.pg_storage import PgEventStorage
...
class NullRawStorage:
    """No-op storage to satisfy adapter interface without writing to disk."""

    @staticmethod
    def day_key(ts: int) -> str:
        return "noop"

    def append(self, key: str, items):  # pragma: no cover - intentionally no-op
        return None
...
async def main() -> None:
    ...
    raw_storage = NullRawStorage()
    latest_store = LatestTelemetryStore(storage_root)
    pg_storage = PgEventStorage()
    ...
    log.info(
        "stream_runner: starting poll loop sources=%s units=%s interval=%ss metrics_interval=%ss latest_metrics=redis",
        source_kinds,
        len(unit_ids),
        args.interval,
    ...
    # track bootstrapped unit_ids to avoid лишние INSERT'ы
    bootstrapped_units: set[int] = set()

    def bootstrap_unit(unit_id: int, name: str | None = None) -> None:
        """Create unit + units_meta(owner_node_id=1) if absent."""
        if unit_id in bootstrapped_units:
            return
        dsn = args.registry_dsn or os.getenv("DEVICE_REGISTRY_DSN") or os.getenv("DATABASE_URL")
        if not dsn:
            return
        try:
            with psycopg.connect(dsn) as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        ...
                        """,
                        (unit_id, name or f"Unit {unit_id}"),
                    )
                    cur.execute(
                        """
                ...
                conn.commit()
            bootstrapped_units.add(unit_id)
            log.info("stream_runner: bootstrapped unit %s", unit_id)
        except Exception as exc:  # pragma: no cover - defensive
            log.warning("stream_runner: bootstrap unit %s failed: %s", unit_id, exc)
    ...
    async def reload_registry_loop():
        """Periodically reload DeviceRegistry to avoid shadow 'boomerangs'."""
        if not registry:
            return
        ...
        while not stop_event.is_set():
            ...
            try:
                count = await loop.run_in_executor(None, registry.load)
                log.debug("stream_runner: registry reloaded entries=%s", count)
            except Exception as exc:  # pragma: no cover - defensive
                log.warning("stream_runner: registry reload failed: %s", exc)
        ...
        try:
            async for msg in adapter_inst.subscribe():
                ...
                if isinstance(msg, Event):
                    # авто-bootstrapping для доверенных источников (Wialon) по unit_id
                    if not registry or not registry.has_unit_id(msg.unit_id):
                        unit_name = str(msg.params.get("name") or msg.params.get("nm") or msg.unit_id)
                        bootstrap_unit(msg.unit_id, unit_name)
                        if registry and registry.enabled:
                            registry.load()

                    try:
                        await loop.run_in_executor(None, pg_storage.store_events, [msg])
                    except Exception as exc:
                        log.warning("stream_runner: pg insert failed: %s", exc)
                    if latest_store:
                        latest_store.update_from_events([msg])
                    continue
                if isinstance(msg, RawPacket):
                    # resolve via registry only; otherwise push to shadow and skip
                    entry = registry.resolve(msg.protocol, msg.uid) if registry else None
                    if registry and entry is None:
                        # Попробуем мгновенно обновить реестр, чтобы новые юниты
                        # не успевали уйти в Shadow между create/bind и reload_loop.
                        try:
                            registry.load()
                            entry = registry.resolve(msg.protocol, msg.uid)
                        except Exception:
                            entry = None
                    unit_id = entry.unit_id if entry else None
                    if unit_id is None:
                        shadow.register(
                            ShadowPacket(
                    ...
                    try:
                        await loop.run_in_executor(None, pg_storage.store_events, [ev])
                    except Exception as exc:
                        log.warning("stream_runner: pg insert failed: %s", exc)
                    if latest_store:
