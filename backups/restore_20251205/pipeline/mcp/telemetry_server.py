...
# -------------------- extra diagnostics tools -------------------- #


@mcp.tool()
def tail_ingest_logs(source: str = "wialon_ips", lines: int = 200) -> Dict[str, Any]:
    """Tail ingest log for a source ('wialon_ips' or 'galileosky')."""

    source = (source or "").lower()
    path = LOGS_DIR / ("wialon_ips_ingest.log" if source == "wialon_ips" else "galileosky_ingest.log")
    count, tail = _read_text_tail(path, max_lines=max(1, min(int(lines), 2000)))
    return {"path": str(path), "lines": count, "content": tail}


@mcp.tool()
def unit_latest_event(unit_id: int) -> Dict[str, Any]:
    """Return latest raw Event for unit_id from data/pipeline_storage/*/events.jsonl."""

    import json as _json
    root = LATEST_METRICS_ROOT
    latest = None
    latest_day = None
    days = sorted([p.name for p in root.iterdir() if p.is_dir() and p.name[:2].isdigit()], reverse=True)
    for day in days:
        fp = root / day / "events.jsonl"
        if not fp.exists():
            continue
        try:
            for line in fp.read_text(encoding="utf-8").splitlines():
                ...
                if rec.get("unit_id") != unit_id:
                    continue
                if latest is None or rec.get("device_ts", 0) >= latest.get("device_ts", 0):
                    latest = rec
                    latest_day = day
        except Exception as exc:  # pragma: no cover - defensive
            return {"found": False, "error": f"failed to read {fp}: {exc}"}
        if latest:
            break
    return {"found": latest is not None, "day": latest_day, "event": latest}


@mcp.tool()
def reload_snapshot_v2() -> Dict[str, Any]:
    """Rebuild snapshot v2 bundle and refresh service cache."""

    try:
        bundle = build_snapshot_v2_bundle()
        svc = UnitSnapshotV2Service(snapshot_path=SNAPSHOT_V2_PATH)
        svc.refresh(bundle.units.values(), source_kind=bundle.source_kind, dump_ts=bundle.dump_ts)
        return {"status": "ok", "units": len(bundle.units), "dump_ts": bundle.dump_ts}
    except Exception as exc:  # pragma: no cover - defensive
        return {"status": "error", "error": str(exc)}


@mcp.tool()
def port_status(ports: Optional[List[int]] = None) -> Dict[str, Any]:
    """Check if given ports are open on localhost (defaults: 8088, 18081, 8080)."""

    import socket

    ports = ports or [8088, 18081, 8080]
    result: Dict[str, str] = {}
    for port in ports:
        try:
