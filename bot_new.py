...
# WLN экспорт отключён в новой архитектуре; импорты убраны
from pipeline.config.defaults import load_from_env as load_pipeline_config
from pipeline.events import Event
from pipeline.config.unit_config import GeneralConfig, SensorConfig, UnitConfig
...
from bot.features import dut as dut_features
from bot.features import dut_admin as dut_admin_features
from bot.features import params as params_features
from bot.features import reports as report_features
...
send_param_prompt = params_features.send_prompt
update_param_prompt = params_features.update_prompt
delete_param_result_message = params_features.delete_param_result_message
clear_param_runtime = params_features.clear_param_runtime
...
# reports: fallback clear_report_context (отчёты выключены, но нужно чистить контекст)
try:
    clear_report_context = report_features.clear_report_context  # type: ignore[attr-defined]
...
log = loggers.app
cancel_log = loggers.cancel
nearby_log = loggers.nearby
dut_cal_log = loggers.dut_cal
message_cache_store_log = loggers.message_cache_store
concurrency_log = loggers.concurrency
message_load_log = loggers.message_load
message_calc_log = loggers.message_calc
token_rotation_log = loggers.token_rotation
geo_log = loggers.geo
...
dut_log = loggers.dut
detector_log = loggers.detector
card_log = loggers.card
snapshot_log = loggers.snapshot
...
UNIT_SNAPSHOT_UPDATER_ENABLED = False  # full rebuild по расписанию отключён (bootstrap из телеметрии)
UNIT_SNAPSHOT_REFRESH_ENABLED = False
UNIT_SNAPSHOT_META_PATH = (BASE_DIR / "data/unit_snapshot.meta.json").resolve()
# Параметры фонового обновления снапшота: по умолчанию раз в час, настраиваются через ENV.
UNIT_SNAPSHOT_TTL = int(os.getenv("UNIT_SNAPSHOT_TTL_SEC", "3600") or 3600)
UNIT_SNAPSHOT_REFRESH_INTERVAL = int(os.getenv("UNIT_SNAPSHOT_REFRESH_INTERVAL_SEC", "3600") or 3600)
UNIT_SNAPSHOT_JITTER = float(os.getenv("UNIT_SNAPSHOT_JITTER", "0.15") or 0.15)
...
def _ensure_unit_item_normalized(raw: Any) -> Dict[str, Any]:
    ...
    sensors = dut_admin_features.normalize_unit_sensors(normalized.get("sens"))
    if sensors:
...
def _fetch_device_meta_with_batches(
    ...
    if not unit_ids:
        return {}
    clients, extra_clients = collect_device_clients(base_client)
    if not clients:
    ...
    try:
        return fetch_device_meta_with_batches(
            base_client,
            clients,
            unit_ids,
            chunk_size=UNIT_DEVICE_BATCH_SIZE,
            retries=UNIT_DEVICE_BATCH_RETRIES,
...
def _build_unit_snapshot_payload(
    ...
    while True:
        ...
        for raw in raw_items:
            ...
            if inline_meta:
                apply_device_meta_fields(minimal_entry, inline_meta)
...
def _search_units_locally(query: str, limit: int = 50) -> List[Dict[str, Any]]:
    """Поиск по локальному снапшоту/индексу.

    Возвращает список словарей с ключами id/nm/pos/lmsg в том виде,
    который ожидают downstream-хелперы поиска/карточки.
    """
    ...
    try:
        ...
        matches = index.search(query_norm, limit=limit) or []
    except Exception:
        matches = []

    results: List[Dict[str, Any]] = []
    for m in matches[:limit]:
        # index entries already имеют id/nm; копируем, чтобы не мутировать кэш
        entry = dict(m)
        uid = entry.get("id")
        if uid is not None:
            try:
...
def _load_snapshot_from_service() -> Tuple[Dict[int, Dict[str, Any]], Optional[int]]:
    try:
        ...
        snapshot_log.debug("unit_snapshot: service load failed: %s", exc, exc_info=True)
        return {}, None
    if not bundle.units:
...
def ensure_unit_snapshot_updater(client: "WialonClient") -> None:
    ...
    with _UNIT_SNAPSHOT_UPDATER_LOCK:
        if _UNIT_SNAPSHOT_UPDATER_STARTED:
            return
        if not client.token:
            log.info("unit_snapshot: cannot start updater without token")
            return
...
async def _activate_objects_ui(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    card_log.debug("card:start_ui ui=objects chat=%s", context._chat_id if hasattr(context, "_chat_id") else None)
...
def _log_nearby(stage: str, *, chat_id=None, unit_id=None, **fields: Any) -> None:
    payload = [f"stage={stage}"]
    if chat_id is not None:
        payload.append(f"chat={chat_id}")
    if unit_id is not None:
        payload.append(f"unit={unit_id}")
...
async def _present_search_results(
    ...
    *,
    ...
    """Единая отрисовка результатов поиска (локальный или Wialon)."""
    if search_token != search_features.current_search_generation(context):
        return _objects_mode_to_state(context)

    card_log.info(
        "card:present_results units=%s token=%s ui=%s anchor=%s",
        len(units or []),
        search_token,
        _active_ui(context),
        _anchor_message_id(context),
    )
    ...
    if len(units) == 1:
        ...
        if client_to_use is None:
            client_to_use = await require_wialon_client(
                update,
                context,
                "Чтобы показать карточку, авторизуйтесь в Wialon.",
            )
    ...
    search_features.set_units(context, units[: search_features.SEARCH_UNITS_LIMIT])
...
async def set_anchor_on(message: Message, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Сохраняет ссылку на сообщение-контейнер для дальнейших правок."""
...
async def edit_anchor(
    ...
    try:
        ...
        log.debug("edit_anchor failed: %s", exc)


async def send_new_anchor_below(
    update: Update,
    text: str,
...
async def delete_stats_map_message(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        log.debug("delete_stats_map_message: %s", exc)
...
async def delete_stats_message(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        log.debug("delete_stats_message: %s", exc)
...
def _build_local_stats_item(unit_id: int) -> Dict[str, Any]:
    """Собрать минимальный item для карточки из локального снапшота и latest_metrics."""
    snapshot = _lookup_unit_snapshot(unit_id) or {}
    ...
    if ts:
        try:
            ...
            ts_int = None
        if ts_int:
            pos.setdefault("t", ts_int)
            lmsg["t"] = ts_int

    params = latest.get("params") or latest.get("prms") or {}

    return {
        "id": unit_id,
        "nm": snapshot.get("nm") or f"id {unit_id}",
        "uid": snapshot.get("uid"),
        "hw": snapshot.get("hw") or snapshot.get("d"),
        "pos": pos,
        "lmsg": lmsg,
        "prms": params,
        "params": params,
...
def _build_map_url(address: Optional[str] = None, lat: Optional[float] = None, lon: Optional[float] = None) -> Optional[str]:
    if lat is not None and lon is not None:
        try:
            lat_f = float(lat)
            lon_f = float(lon)
            return f"https://maps.google.com/?q={lat_f:.6f},{lon_f:.6f}"
        except Exception:
            pass
    if address:
...
def _is_online(last_ts: Optional[int]) -> bool:
    """True, если последнее сообщение свежее ONLINE_THRESHOLD_SEC."""
    if not last_ts:
        return False
    now = datetime.now(timezone.utc)
...
def _build_dut_lines2(
    ...
    *,
    ...
    main_sensor = select_fuel_sensor(unit_item)
...
async def show_stats_then_actions(
    ...
    *,
    ...
    card_log.info(
        "card:stats_start unit_id=%s unit_name=%s via_callback=%s preserve_previous=%s client=%s",
        unit_id,
        unit_name,
        via_callback,
        preserve_previous,
    ...
    def _restore_previous_stats_meta() -> None:
        if preserve_previous and prev_stats_meta and "stats_msg" not in context.user_data:
    ...
    # Источник данных: локальный pipeline, без Wialon (используем Wialon только если pipeline отключён)
    use_local = _pipeline_card_enabled()
    if use_local:
        try:
            item = _build_local_stats_item(unit_id)
        except Exception as exc:
            _restore_previous_stats_meta()
            log.exception("Ошибка загрузки локального item (Stats)")
            card_log.error("card:stats_error unit_id=%s err=%s", unit_id, exc)
            target = update_or_q.message
            await target.reply_text(
                "Не удалось загрузить объект из локального кэша. Обновите снапшот/поток и попробуйте снова.",
                reply_markup=kb_cancel("back:find"),
            )
            return STATE_FIND_QUERY
    else:
        try:
            item = await run_blocking(client.get_unit_full_for_stats, unit_id)  # type: ignore[arg-type]
        except Exception as exc:
            _restore_previous_stats_meta()
            ...
            target = update_or_q.message
            await target.reply_text(
                f"Не удалось загрузить объект: `{exc}`",
                reply_markup=kb_cancel("back:find"),
            )
            return STATE_FIND_QUERY
    ...
    # Если Wialon не вернул параметры, пробуем подтянуть последнее состояние из локального latest_metrics
    if not params_map:
        try:
            latest = get_pipeline_storage_service().get_latest_metrics(unit_id)
        except Exception as exc:
            latest = None
            log.debug("latest_metrics fetch failed: %s", exc)
        if isinstance(latest, dict):
            metrics_params = latest.get("params") or latest.get("prms") or {}
            if isinstance(metrics_params, dict) and metrics_params:
                params_map = dict(metrics_params)
                params_order = list(metrics_params.keys())
                card_log.info("card:params_filled_from_latest unit_id=%s keys=%s", unit_id, len(params_map))
    ...
    if not use_local and client:
        ...
        if sensor:
            ...
            try:
                await run_blocking(client.load_last_message, unit_id)
                value = await run_blocking(client.calc_sensor_value, unit_id, sensor_id)
                ...
                log.warning(f"Не удалось получить значение ДУТ: {exc}")
    # Фоллбек: если нет Wialon-значения, попробуем вычислить из UnitConfig/params_map
    sensor_values: Optional[Dict[str, Tuple[Optional[float], Optional[float]]]] = None
    if unit_config:
        try:
            sensor_values = card_features.compute_unit_config_sensor_values(unit_config, params_map)
            fuel_from_uc = card_features.pick_unit_config_fuel_display(unit_config, sensor_values)
            if fuel_from_uc:
                fuel_line = fuel_from_uc
        except Exception as exc:
            log.debug("unitconfig fuel calc failed: %s", exc)
    ...
    if not address_text and lat_val is not None and lon_val is not None:
        try:
            geocoded = await run_blocking(client.reverse_geocode, lat_val, lon_val)
        except Exception as exc:
            geocoded = None
    ...
    if lat_val is not None and lon_val is not None:
        try:
            geozone_payload = await run_blocking(
                client.get_unit_geozone_details, unit_id, lat_val, lon_val
            )
        except Exception as exc:
            geozone_payload = {}
    ...
    snapshot_entry = _lookup_unit_snapshot(unit_id) or {}
    snapshot_record = None  # совместимость с card_features; при отсутствии полноценного объекта не заполняем
    snapshot_line: Optional[str] = None
    dump_ts = snapshot_entry.get("dump_ts") or snapshot_entry.get("t")
    if dump_ts:
        snapshot_line = f"• Снапшот: unit_snapshot · {_fmt_ts_utc(dump_ts)}"

    try:
        device_dut_lines = await run_blocking(_build_dut_lines2, client, item, unit_id)
    except Exception as exc:
        device_dut_lines = []
        log.warning("build_dut_lines failed: %s", exc)

    # Собираем карточку через card_features для более полного вывода
    card_layout = card_features.build_card_layout(
        unit_name=unit_name,
        reg_number=None,
        online_text=online_text,
        satellites_text=f"{sat_dot} определены {sat_ago} назад ({sats_text})",
        last_message_text=_fmt_ts_utc(last_ts),
        fuel_text=fuel_line,
        driver_text=None,
        coords_text=coords_text,
        address_hint=address_text,
        snapshot_record=snapshot_record,
        device_dut_lines=device_dut_lines,
        info_wialon_id=str(unit_id),
        info_device_type=str(item.get("hw") or item.get("d") or "—"),
        info_terminal_id=str(item.get("uid") or "—"),
        unit_config=unit_config,
        params_map=params_map,
        sensor_values=sensor_values,
        location_display=location_display,
        location_map_url=location_map_url,
        geozone_display=geozone_display,
        geozone_map_url=geozone_map_url,
    )

    lines = list(card_layout.lines)
    links_map = dict(card_layout.links)

    # Добавляем блок параметров (первые 10)
    params_entries: List[str] = []
    if params_map:
        for key in list(params_map.keys())[:10]:
            try:
                val = params_map.get(key)
                params_entries.append(f"• {key}: {val}")
            except Exception:
                continue
        if params_entries:
            card_features.append_group(lines, "⚙️ Параметры", params_entries)

    # Добавляем строку про снапшот, если есть
    if snapshot_line:
        card_features.append_group(lines, "📦 Снапшот", [snapshot_line])

    message_html = card_features.format_lines_with_links(lines, links_map)
    target = update_or_q.message
    ...
    card_log.info("card:render_done unit_id=%s lines=%s", unit_id, len(lines))
...
async def _set_export_anchor_on(message: Message, context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def _handle_nearby_pipeline(
    ...
    def _log(stage: str, **fields: Any) -> None:
        _log_nearby(stage, chat_id=chat_id, unit_id=unit_id, trace=trace_id, **fields)
...
async def reports_entry(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not client:
        return STATE_AUTH_WAIT_TOKEN
    return await report_features.reports_entry(update.message, context, client)
...
async def cmd_web(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    url = WEB_BASE_URL or "http://localhost:8000"
    text = f"Открыть веб-версию: {url}"
    if update.message:
        await update.message.reply_text(text, disable_web_page_preview=False)
    return ConversationHandler.END


async def web_button_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
...
def _format_pipeline_summary(day: str, summary: Dict[str, Any], storage_root: Path) -> str:
    ...
    if first_ts and last_ts:
        lines.append(
            f"UTC: {datetime.fromtimestamp(first_ts, tz=timezone.utc)} → {datetime.fromtimestamp(last_ts, tz=timezone.utc)}"
        )
...
async def stats_actions_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == "add_dut":
        ...
        return await add_dut_start(q, context)

    if action == "sensors":
        return await show_sensor_settings(q, context)

    if action == "dutcal":
        return await show_dut_calibration(q, context)

    if action == "dutcal_refresh":
        return await refresh_dut_calibration(q, context)

    if action == "clear_sensors":
        return await clear_unit_sensors(q, context)

    if action == "report":
        await _safe_answer_callback(q)
        clear_report_context(context, cancel_job=True)
        await q.message.reply_text("Отчёты временно отключены.")
        return STATE_WAIT_UNIT
    ...
    if action == "nearby":
        ...
        if not unit:
            await q.message.reply_text("Сначала выберите объект.")
            return STATE_WAIT_UNIT
        try:
            return await _handle_nearby_pipeline(
                q,
                context,
                int(unit.get("id")),
                str(unit.get("nm")),
            ...
            log.exception("nearby failed: %s", exc)
            await q.message.reply_text("Ближайшие объекты временно недоступны.")
            return STATE_WAIT_UNIT
...
async def cancel_inline_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # По умолчанию: вернуть пользователя в карточку, если выбран объект; иначе в поиск
    unit = context.user_data.get("chosen_unit")
    if unit:
        try:
            ...
            unit_id = None
        unit_name = unit.get("nm") or (f"id {unit_id}" if unit_id is not None else "объект")
        await _activate_objects_ui(context)
        _set_objects_mode(context, OBJECTS_MODE_CARD_IDLE)
        await delete_param_result_message(context)
        clear_param_runtime(context)
        ...
        if not _pipeline_card_enabled():
            client = await require_wialon_client(
                update,
                context,
                "Чтобы вернуться к карточке, авторизуйтесь.",
            )
            if not client:
                return STATE_AUTH_WAIT_TOKEN
        return await show_stats_then_actions(
    ...
    # Если объекта нет — в поиск
    await _activate_objects_ui(context)
    _set_objects_mode(context, OBJECTS_MODE_LIST)
    await _start_search_prompt_from_trigger(q, context)
    return ConversationHandler.END
...
async def back_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if code == "unit":
        ...
        if not unit:
            ...
            if not units:
                anchor = context.user_data.get("anchor")
                prompt = "Введите имя/госномер для поиска…"
                if anchor:
                    await edit_anchor(context, prompt, kb_search_prompt(context))
                elif q.message:
                    msg = await q.message.reply_text(prompt, reply_markup=kb_search_prompt(context))
                    await set_anchor_on(msg, context)
                return STATE_FIND_QUERY
    ...
    if code in ("cmd_unit", "name", "value"):
        # simplified behavior: reset to menu
        context.user_data["mode"] = MODE_NONE
        await delete_anchor(context)
        await delete_stats_message(context)
        await delete_param_result_message(context)
...
async def clear_stats_buttons(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        log.debug("clear_stats_buttons: %s", exc)
...
async def restore_stats_buttons(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        log.debug("restore_stats_buttons: %s", exc)


async def remove_reply_markup_safe(message: Optional[Message]) -> None:
    if not message:
        return
    try:
        await message.edit_reply_markup(reply_markup=None)
    except Exception as exc:
        log.debug("remove_reply_markup_safe: %s", exc)
...
async def remove_reply_markup_by_meta(context: ContextTypes.DEFAULT_TYPE, meta: Optional[Dict[str, Any]]) -> None:
    ...
    try:
        ...
        log.debug("remove_reply_markup_by_meta: %s", exc)
...
# Legacy placeholders (не используются в UI, оставлены для совместимости зависимостей)
STATE_CMD_VALUE = 60
...
def kb_search_prompt(context: ContextTypes.DEFAULT_TYPE) -> InlineKeyboardMarkup:
    # Минимальный промпт поиска: только «Отмена» (или «Назад», если задано).
    return kb_cancel()
...
def _get_search_units(context: ContextTypes.DEFAULT_TYPE) -> List[Dict[str, Any]]:
    return search_features.get_units(context)


def _filtered_units(context: ContextTypes.DEFAULT_TYPE) -> List[Dict[str, Any]]:
    return search_features.filtered_units(context)


def _reset_search_context(context: ContextTypes.DEFAULT_TYPE) -> None:
    search_features.reset_search_context(
        context,
        extra_keys=["stats_unit_item"],
        after_reset=_clear_stats_generation,
    )
...
async def _render_units_page(
    ...
    *,
    ...
    normalized_offset = search_features.normalize_units_offset(total, int(offset))
    context.user_data["search_units_offset"] = normalized_offset
    if total:
        markup = search_features.kb_units_page(
            units, normalized_offset, status_marker=_unit_status_markers
        )
    else:
        markup = kb_search_prompt(context)
    text = search_features.list_header(total)
    anchor = context.user_data.get("anchor")
...
async def _handle_objects_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    card_log.info(
        "card:text ui=%s mode=%s text=%s chat=%s",
        _active_ui(context),
        _objects_mode(context),
    ...
    search_token = search_features.bump_search_generation(context)
    ...
    if len(text) < 3:
        units = search_features.get_units(context)
        search_features.set_quick_mask(context, text)
        if units:
            context.user_data["search_units_offset"] = 0
            _set_objects_mode(context, OBJECTS_MODE_LIST)
            await _render_units_page(
    ...
    if _pipeline_search_enabled():
        try:
            units = await run_blocking(
                _search_units_locally, text, search_features.SEARCH_UNITS_LIMIT
            )
        except Exception as exc:
    ...
    client = await require_wialon_client(
        update,
        context,
        "Чтобы искать объекты, авторизуйтесь в Wialon.",
    )
    if not client:
        return STATE_AUTH_WAIT_TOKEN
    ...
    try:
        units = await run_blocking(
            client.search_units, text, limit=search_features.SEARCH_UNITS_LIMIT
        )
    except Exception as exc:
        if search_token != search_features.current_search_generation(context):
...
async def choose_unit_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    context.user_data["chosen_unit"] = {"id": unit_id, "nm": selected.get("nm", f"id {unit_id}")}
    _set_objects_mode(context, OBJECTS_MODE_CARD_IDLE)
    await delete_anchor(context)
    ...
    return await show_stats_then_actions(
        ...
        context,
        unit_id,
        context.user_data["chosen_unit"].get("nm", f"id {unit_id}"),
        client,
        via_callback=True,
        preserve_previous=True,
    )

STATS_ACTIONS_PATTERN = r"^stats:(refresh|params|report|nearby|to_list|add_dut|sensors|dutcal|dutcal_refresh|clear_sensors)$"
...
async def report_name_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if exact:
        ...
        label = report_features.format_report_label(exact)
        text = f"Выбран отчёт: {label}\n\nВыберите период формирования:".strip()
        await send_new_anchor_below(update, text, report_features.kb_report_periods(), context, parse_mode=None)
        return STATE_REPORT_PERIOD

    if matches:
        text = "Совпадения не найдены точно. Выберите отчёт из списка:"
        markup = report_features.store_report_candidates(context, matches, text, offset=0)
        await send_new_anchor_below(update, text, markup, context, parse_mode=None)
        return STATE_REPORT_NAME

    if templates:
        text = "❌ Совпадений не найдено. Ниже доступные отчёты:"
        markup = report_features.store_report_candidates(context, templates, text, offset=0)
        await send_new_anchor_below(update, text, markup, context, parse_mode=None)
    else:
        await send_new_anchor_below(
            update,
...
async def report_choose_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if data.startswith("report:more:"):
        ...
        if not matches:
            ...
            return STATE_REPORT_NAME
        markup = report_features.build_report_candidates_markup(context, offset=offset)
    ...
    context.user_data.pop("report_offset", None)
    context.user_data.pop("report_candidates_text", None)
    label = report_features.format_report_label(selected)
    text = (
        "Выбран отчёт: {label}\n\n"
        "Выберите период формирования или нажмите «🔁 Другой шаблон», чтобы вернуться к списку."
    ).format(label=label)
    await edit_anchor(context, text, report_features.kb_report_periods(), parse_mode=None)
    return STATE_REPORT_PERIOD
...
async def report_change_template_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    text, markup = report_features.build_report_start_prompt(context, templates, mask)
...
async def report_back_to_period_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    label = report_features.format_report_label(template)
    context.user_data.pop("report_period", None)
    text = f"Отчёт: {label}\n\nВыберите период формирования:".strip()
    await edit_anchor(context, text, report_features.kb_report_periods(), parse_mode=None)
    return STATE_REPORT_PERIOD
...
async def report_period_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    tpl_label = report_features.format_report_label(template)
    text = (
        f"Отчёт: {tpl_label}\n"
        f"Период: {label}\n\n"
        "Выберите формат выгрузки:"
    )
    await edit_anchor(context, text, report_features.kb_report_formats(), parse_mode=None)
...
async def report_format_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await edit_anchor(
            ...
            report_features.kb_report_periods(),
            parse_mode=None,
        )
        return STATE_REPORT_PERIOD
    ...
    format_title = report_features.describe_report_format(canonical_fmt)
    tpl_label = report_features.format_report_label(sanitized_template)
    ...
    await edit_anchor(context, text, report_features.kb_report_wait(), parse_mode=None)
...
async def report_cancel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    context.user_data.pop("anchor", None)
    await delete_stats_message(context)
    await params_features.delete_param_result_message(context)
    params_features.clear_param_runtime(context)
    context.user_data.pop("report_job", None)
    clear_report_context(context, cancel_job=False)
    if q.message:
        await q.message.reply_text(
...
async def report_generation_worker(context: ContextTypes.DEFAULT_TYPE, job: Dict[str, Any]) -> None:
    ...
    try:
        ...
        summary_lines.extend(
            [
                f"Отчёт: {report_features.format_report_label(template)}",
                f"Период: {period.get('label', '')}",
                f"Формат: {report_features.describe_report_format(fmt)}",
            ]
        )
        summary = "\n".join(summary_lines)
        ...
        await edit_anchor(
            context,
            f"{summary}\n\n✅ Отчёт готов.",
            markup=report_features.kb_report_finish(has_unit),
            parse_mode=None,
        )
        context.user_data.pop("anchor", None)
...
params_handler_deps = params_features.ParamsHandlerDeps(
    ensure_authorized=ensure_authorized,
    safe_answer_callback=_safe_answer_callback,
    send_prompt_cb=send_param_prompt,
    update_prompt_cb=update_param_prompt,
    delete_anchor_cb=delete_anchor,
    set_anchor_cb=set_anchor_on,
    delete_stats_message=delete_stats_message,
    clear_stats_buttons=clear_stats_buttons,
    remove_reply_markup_safe=remove_reply_markup_safe,
    remove_reply_markup_by_meta=remove_reply_markup_by_meta,
    kb_cancel=kb_cancel,
    kb_cmd_entry_controls=kb_cmd_entry_controls,
    reply_menu=reply_menu,
    require_wialon_client=require_wialon_client,
    pipeline_card_enabled=_pipeline_card_enabled,
    show_stats_then_actions=show_stats_then_actions,
    clear_export_job=_clear_export_job,
    state_menu=STATE_MENU,
    state_param_query=STATE_PARAM_QUERY,
    state_auth_wait_token=STATE_AUTH_WAIT_TOKEN,
    state_find_query=STATE_FIND_QUERY,
    state_cmd_value=STATE_CMD_VALUE,
    mode_params=MODE_PARAMS,
    mode_none=MODE_NONE,
    mode_cmd=MODE_CMD,
)
params_query_text = params_features.build_params_query_handler(params_handler_deps)
params_buttons_cb = params_features.build_params_buttons_handler(params_handler_deps)

sensor_handler_deps = sensor_features.SensorsFeatureDeps(
    safe_answer_callback=_safe_answer_callback,
    load_unit_config=_load_unit_config,
    exports_dir=SENSOR_EXPORTS_DIR,
    state_wait_unit=STATE_WAIT_UNIT,
)
show_sensor_settings = sensor_features.build_show_sensor_settings_handler(sensor_handler_deps)

clear_sensors_deps = sensor_features.ClearSensorsDeps(
    safe_answer_callback=_safe_answer_callback,
    pipeline_card_enabled=_pipeline_card_enabled,
    ensure_unit_config_object=_ensure_unit_config_object,
    save_unit_config=lambda cfg: get_unit_config_service().save(cfg),
    clear_unit_config_cache=_clear_unit_config_cache,
    exports_dir=SENSOR_EXPORTS_DIR,
    show_stats_then_actions=show_stats_then_actions,
    state_wait_unit=STATE_WAIT_UNIT,
)
clear_unit_sensors = sensor_features.build_clear_sensors_handler(clear_sensors_deps)

dut_view_deps = dut_features.DutCalibrationViewDeps(
    safe_answer_callback=_safe_answer_callback,
    load_unit_config=_load_unit_config,
    state_wait_unit=STATE_WAIT_UNIT,
    log=log,
...
show_dut_calibration = dut_features.build_show_calibration_handler(dut_view_deps)

dut_refresh_deps = dut_features.DutCalibrationRefreshDeps(
    safe_answer_callback=_safe_answer_callback,
    ensure_unit_config_object=_ensure_unit_config_object,
    save_unit_config=lambda cfg: get_unit_config_service().save(cfg),
    clear_unit_config_cache=_clear_unit_config_cache,
    wialon_for_chat=wialon_for_chat,
    not_authorized_exc=NotAuthorized,
    user_blocked_exc=UserBlocked,
    run_blocking=run_blocking,
    log=log,
    dut_log=dut_cal_log,
    state_wait_unit=STATE_WAIT_UNIT,
    show_calibration_handler=show_dut_calibration,
)
refresh_dut_calibration = dut_features.build_refresh_calibration_handler(dut_refresh_deps)

add_dut_start = dut_admin_features.build_add_dut_start_handler(
    dut_admin_features.AddDutStartDeps(
        safe_answer_callback=_safe_answer_callback,
        kb_cancel=kb_cancel,
        state_wait_unit=STATE_WAIT_UNIT,
        state_add_dut_wait_name=STATE_ADD_DUT_WAIT_NAME,
        log=log,
    )
)
add_dut_name_text = dut_admin_features.build_add_dut_name_handler(
    dut_admin_features.AddDutNameDeps(
        ensure_authorized=ensure_authorized,
        require_wialon_client=require_wialon_client,
        run_blocking=run_blocking,
        pipeline_card_enabled=_pipeline_card_enabled,
        ...
        ensure_unit_config_object=_ensure_unit_config_object,
        save_unit_config=lambda cfg: get_unit_config_service().save(cfg),
        export_calibration_csv=lambda unit_id, sensor: sensor_features.export_sensor_calibration_csv(
            SENSOR_EXPORTS_DIR, unit_id, sensor
        ),
        state_wait_unit=STATE_WAIT_UNIT,
        state_auth_wait_token=STATE_AUTH_WAIT_TOKEN,
        state_add_dut_wait_name=STATE_ADD_DUT_WAIT_NAME,
...
def main() -> None:
    ...
    async def handle_error(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
        # Глобальный трап ошибок, чтобы падения не оставались немыми и не прерывали диалог
        log.exception("unhandled error: %s", context.error, exc_info=context.error)
        chat_id: Optional[int] = None
        ...
        if chat_id:
            try:
                await context.bot.send_message(
                    chat_id=chat_id,
                    text="Что-то пошло не так. Попробуйте ещё раз, либо отправьте /start.",
                )
            except Exception:
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            STATE_MENU: [
                ...
                MessageHandler(filters.Regex(f"^{re.escape(MENU_BUTTON_SETTINGS)}$"), settings_entry),
                MessageHandler(filters.Regex(r"^🌐 WEB$"), web_button_text),
                CallbackQueryHandler(start_action_cb, pattern=r"^start:find$"),
    ...
    log.info(
        "Бот запущен: entry-point для текста активен, allow_reentry=False; legacy CF/CMD/EXPORT отключены."
    )
