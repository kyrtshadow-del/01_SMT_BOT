...
async def api_history_trips(unit_id: int, from_ts: int, to_ts: int, session_id: str = Depends(get_session_id)) -> List[TripResponse]:
    # проверка сессии (пока без фильтра прав)
    _get_access_context(session_id)
    query = """
        SELECT id, unit_id, EXTRACT(EPOCH FROM start_ts)::bigint AS start_ts, EXTRACT(EPOCH FROM end_ts)::bigint AS end_ts,
               type, distance_m, max_speed, start_address, end_address
        FROM trips
        WHERE unit_id = %s
...
async def api_history_track(unit_id: int, from_ts: int, to_ts: int, session_id: str = Depends(get_session_id)) -> List[TrackPoint]:
    _get_access_context(session_id)
    query = """
