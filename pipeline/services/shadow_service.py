...
class ShadowService:
    def __init__(self, redis_url: Optional[str] = None, ttl_sec: int = 24 * 3600) -> None:
        ...
        if not self.enabled:
            ...
            log.info("shadow_service: enabled url=%s", self.redis_url)
    ...
    # ----------- helpers for UI/registry ----------
    def list_keys(self, pattern: str = "shadow:device:*", limit: int = 200) -> list[str]:
        if not self.enabled or self._client is None:
            return []
        keys: list[str] = []
        try:
            cursor = 0
            while True:
                cursor, batch = self._client.scan(cursor=cursor, match=pattern, count=min(limit, 200))
                keys.extend([k.decode() if isinstance(k, bytes) else k for k in batch])
                if cursor == 0 or len(keys) >= limit:
                    break
        except Exception as exc:  # pragma: no cover - defensive
            log.warning("shadow_service: scan failed: %s", exc)
        return keys[:limit]
    ...
    def get(self, key: str) -> dict | None:
        ...
        try:
            ...
            def _decode(val):
                ...
                return val
            return {k.decode() if isinstance(k, bytes) else k: _decode(v) for k, v in raw.items()}
        except Exception as exc:  # pragma: no cover - defensive
            log.warning("shadow_service: get failed %s: %s", key, exc)
            return None
    ...
    def delete(self, key: str) -> None:
        ...
        try:
            ...
            log.warning("shadow_service: delete failed %s: %s", key, exc)

    def get_count(self) -> int:
        """Count shadow devices. For small N scan_iter is acceptable."""
        if not self.enabled or self._client is None:
            return 0
        try:
            count = 0
            for _ in self._client.scan_iter("shadow:device:*"):
                count += 1
            return count
        except Exception as exc:  # pragma: no cover - defensive
            log.warning("shadow_service: count failed: %s", exc)
            return 0
