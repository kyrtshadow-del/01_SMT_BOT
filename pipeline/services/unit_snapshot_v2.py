...
try:  # optional, as в других местах
    import psycopg
...
def _load_registry_unit_ids() -> Optional[set[int]]:
    """Return unit_ids from DeviceRegistry or None if registry is unavailable.

    Semantics:
    - Registry disabled / not configured / load failed → return None (builder
      продолжает работать только по локальным данным).
    - Registry успешно загружен, даже если записей 0 → вернуть set(), чтобы
      снапшот явно отражал «0 известных юнитов» и не создавал их только по
      факту наличия событий.
    """
    ...
    try:
        ...
        return None
    return {entry.unit_id for entry in registry.iter_entries()}


def _load_registry_names(unit_ids: set[int]) -> Dict[int, str]:
    """Fetch unit names directly from Registry (Postgres)."""
    ...
    dsn = (
        ...
        or os.getenv("PGDATABASE")  # allow PG* env
        or None
    )
    if not dsn:
        return {}
    try:
        with psycopg.connect(dsn) as conn:
            ...
            cur.execute(f"SELECT id, name FROM units WHERE id IN ({placeholders})", list(unit_ids))
            rows = cur.fetchall()
            return {int(uid): (name or f"unit {uid}") for uid, name in rows}
    except Exception:
        return {}
...
def build_snapshot_v2_bundle() -> UnitSnapshotV2Bundle:
    ...
    # Базовый набор юнитов: те, для кого есть локальный UnitConfig,
    # последние метрики или legacy‑снапшот (имя/устройство из старых данных).
    base_unit_ids = set(configs.keys()) | set(latest.keys()) | set(legacy_names.keys())

    # Если включён DeviceRegistry, дополнительно ограничиваем список юнитов
    # только теми, что реально присутствуют в Registry. Это устраняет
    # противоречие с Ghost/Shadow‑концепцией: в Monitoring попадают только
    # зарегистрированные юниты, а сырые IMEI без линка живут в Shadow.
    registry_unit_ids = _load_registry_unit_ids()
    if registry_unit_ids is not None:
        # Registry включён: доверяем реестру как источнику истины.
        # Даже если у юнита ещё нет локальных файлов (config/metrics), но он есть
        # в Registry, показываем его "голым", чтобы сразу увидеть в списке.
        unit_ids = registry_unit_ids
    else:
        # Registry недоступен: используем только локальные источники
        # (UnitConfig + latest_metrics + legacy snapshot).
        unit_ids = base_unit_ids
    if not unit_ids:
        return UnitSnapshotV2Bundle.empty()

    registry_names = _load_registry_names(unit_ids)
    ...
    for uid in sorted(unit_ids):
        ...
        if cfg_payload:
            ...
            name = registry_names.get(uid) or legacy.get("name") or legacy.get("nm") or f"unit {uid}"
