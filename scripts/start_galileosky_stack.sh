...
# Preserve explicit CLI overrides before loading .env (so SKIP_TUNNEL=0, WEB_PORT override are not clobbered)
CLI_SKIP_TUNNEL="${SKIP_TUNNEL-}"
CLI_WEB_PORT="${WEB_PORT-}"

# Load .env if present (helps to pass REDIS_URL/DEVICE_REGISTRY_* without exporting each time)
if [[ -f "${ROOT}/.env" ]]; then
  echo "[stack] loading ${ROOT}/.env"
  set -a
...
# Reapply CLI overrides if they were passed
if [[ -n "${CLI_SKIP_TUNNEL}" ]]; then
  SKIP_TUNNEL="${CLI_SKIP_TUNNEL}"
fi
# Для веба: по умолчанию всегда 8000, .env не затирает; меняем только если явно передали CLI WEB_PORT
...
# Ensure web port is free (default 8000; override with WEB_PORT if needed)
