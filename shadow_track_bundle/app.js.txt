    ...
    if (showBlock("connectivity")) {
      ...
      sectionBlocks.push({
        ...
        body: renderKvGrid([
          ...
          ...(block.firmware ? [{ label: "Прошивка", value: firmwareValue }] : []),
          ...(block.phones?.length ? [{ label: "Телефоны", value: block.phones.join(", ") }] : []),
        ]),
      });
    }
...
function initModal() {
  ["modal-name", "modal-uid", "modal-hw"].forEach((id) => {
    ...
    if (el) el.addEventListener("input", applyModalFilter);
  });
  const showMap = document.getElementById("modal-show-map");
  if (showMap) showMap.addEventListener("change", (e) => {
  ...
  if (addFound) addFound.addEventListener("click", () => {
...
function handleCardClick(e) {
  ...
  if (action === "card-close") {
    ...
    if (currentCardDetail?.item?.id) {
      openHistorySidebar(currentCardDetail.item);
    }
...
function initHistory() {
  const btnBack = document.getElementById("history-back");
  if (btnBack) btnBack.addEventListener("click", exitHistoryMode);
  const prev = document.getElementById("hist-prev");
  if (prev) prev.addEventListener("click", () => shiftHistoryDate(-1));
  const next = document.getElementById("hist-next");
  if (next) next.addEventListener("click", () => shiftHistoryDate(1));
  const refresh = document.getElementById("history-refresh-btn");
  if (refresh) refresh.addEventListener("click", loadHistoryTrips);
  const dateInput = document.getElementById("hist-date-picker");
  if (dateInput) {
    dateInput.valueAsDate = new Date();
    dateInput.addEventListener("change", loadHistoryTrips);
  }
}

function openHistorySidebar(item) {
  histUnitId = item.id;
  const nameEl = document.getElementById("history-unit-name");
  if (nameEl) nameEl.textContent = item.name || `Unit ${item.id}`;
...
function shiftHistoryDate(delta) {
  ...
  if (!input || !input.valueAsDate) return;
...
async function loadHistoryTrips() {
  ...
  if (listEl) listEl.innerHTML = "Загрузка...";
  clearHistoryTrack();
  const day = input?.valueAsDate || new Date();
  ...
  try {
    ...
    if (sumEl) {
      const dist = (trips || []).filter(t=>t.type==='trip').reduce((s,t)=>s+(t.distance_m||0),0);
      const maxSpd = (trips||[]).reduce((m,t)=>Math.max(m, t.max_speed||0),0);
      sumEl.innerHTML = `Пробег ${(dist/1000).toFixed(1)} км · Макс ${maxSpd} км/ч`;
    }
  } catch (e) {
    if (listEl) listEl.innerHTML = "Ошибка";
  }
}
...
function renderHistoryList(trips) {
  ...
  trips.forEach((t) => {
    ...
    row.innerHTML = `<div><b>${start} - ${end}</b> · ${t.duration_str}</div><div>${t.type==='trip' ? ((t.distance_m/1000).toFixed(1)+' км · '+t.max_speed+' км/ч') : 'Остановка'}</div>`;
    row.addEventListener("click", ()=> loadTrack(t));
    listEl.appendChild(row);
  });
}

async function loadTrack(trip) {
  clearHistoryTrack();
  if (!trip) return;
  try {
    const res = await authFetch(`/web/api/history/track?unit_id=${trip.unit_id}&from_ts=${trip.start_ts}&to_ts=${trip.end_ts}`);
    const pts = await res.json();
    if (!pts.length) { showToast("Нет точек"); return; }
    const latlngs = pts.map(p=>[p.lat,p.lon]);
    histLayerGroup = L.layerGroup().addTo(map);
    L.polyline(latlngs,{color:'#3b82f6',weight:4}).addTo(histLayerGroup);
    L.marker(latlngs[0]).addTo(histLayerGroup);
    L.marker(latlngs[latlngs.length-1]).addTo(histLayerGroup);
    map.fitBounds(L.latLngBounds(latlngs),{padding:[40,40]});
  } catch(e) { console.error(e); showToast("Ошибка трека"); }
}

function clearHistoryTrack() {
  if (histLayerGroup) { map.removeLayer(histLayerGroup); histLayerGroup=null; }
}
