...
class GalileoskyStreamAdapter(StreamAdapter, MetricsMixin):
    ...
    def _parse_payload(self, payload: Any) -> List[Event]:
    ...
    def _build_event(
        ...
        unit_id = self._resolve_unit_id(uid)
        synthetic_unit = False
        if unit_id is None:
            imei = payload.get("imei") or params.get("imei")
            if imei:
                unit_id = self._synthetic_unit_id(str(imei))
                synthetic_unit = True
                params.setdefault("synthetic_unit", True)
                params.setdefault("imei", imei)
            else:
                log.debug("galileosky: skip message uid=%s (unit not found)", uid)
                return None
    ...
    def _persist(self, events: List[Event]) -> None:
        if not events:
            return
        self.raw_storage.append(self._day_key(events[0].device_ts), events)
        if self.latest_store is None:
            # create on the fly if not provided
            self.latest_store = LatestTelemetryStore(self.raw_storage.root)
    ...
    def _resolve_unit_id(self, uid: Any) -> Optional[int]:
        if uid is None:
            return None
        snap = self._snapshot
        if snap is None:
            return None
        bundle = snap.load_bundle()
        target = str(uid)
        for unit_id, rec in bundle.units.items():
            if rec.device.uid and str(rec.device.uid) == target:
                return unit_id
            if getattr(rec.device, "imei", None) and str(rec.device.imei) == target:
                return unit_id
        return None

    @staticmethod
    def _synthetic_unit_id(imei: str) -> int:
        # stable positive int from IMEI
        import zlib
        return zlib.crc32(imei.encode()) & 0x7FFFFFFF
