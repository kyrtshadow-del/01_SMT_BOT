...
def _get_snapshot_bundle():
    """Load only v2 snapshot; no legacy fallback."""
    v2 = get_unit_snapshot_v2_service().load_bundle()
    if v2 and v2.units:
        return v2
    raise HTTPException(status_code=503, detail="unit snapshot v2 is empty; build it first")
...
async def units_feed(
    ...
    return UnitFeedResponse(ts=now_ts, updates=updates, reset=False)
