...
class UnitSnapshotRecord:
    ...
    def from_dict(cls, unit_id: int, payload: Dict[str, Any]) -> "UnitSnapshotRecord":
        ...
        return cls(
            ...
            unconfigured=bool(payload.get("unconfigured", False)),
        )
...
class UnitSnapshotService:
    ...
    def has_unit(self, unit_id: int) -> bool:
        ...
        return unit_id in bundle.units

    def upsert_unit(self, record: UnitSnapshotRecord) -> None:
        """Merge single unit into snapshot and persist to disk."""
        # Critical: load existing bundle *outside* the lock to avoid self-deadlock.
        # load_bundle() acquires the same lock, so calling it inside the locked
        # section would block forever. We only guard the in-memory update +
        # write with the lock.
        bundle = self.load_bundle()
        with self._lock:
            bundle.units[record.unit_id] = record
            bundle.dump_ts = int(time.time())
